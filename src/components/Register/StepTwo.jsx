...
const StepTwo = ({nextStep}) => {
  ...
  return (
    ...
    <form>
      ...
      <label>
        <p>Dirección</p>
        <input type="email" className='input' placeholder='Ingresa una dirección completa' />
      </label>
      <label>
        <p>País de residencia</p>
        <select name="country" id="country" className='input' placeholder='Selecciona uno' value={country} onChange={(event)=>setCountry(event.target.value)}>
          {selectOptions.map((option)=>option)}
        </select>
      </label>
    </form>
    <StyledButton type="submit" case="primary" style={{marginTop:'52px'}} onClick={nextStep}>Guardar y continuar</StyledButton>
