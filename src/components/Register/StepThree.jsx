...
const StepThree = ({nextStep,values,setValues}) => {
  const [creditCard, setCreditCard] = useState(values.creditCard)
  const [cardCode, setCardCode] = useState(values.cardCode)

  const submit = () =>{
    if (!creditCard || !cardCode){
      return
    }
    setValues({...values,creditCard,cardCode})
    nextStep();
  }
  return (
    ...
    <form>
      <label>
        ...
        <input type="number" className='input' placeholder='Ingresa una tarjeta valida' value={creditCard} onChange={(e)=>setCreditCard(e.target.value)} />
      </label>
      <label>
        <p>Código secreto</p>
        <input type="number" className='input' placeholder='CVC/CVV' value={cardCode} onChange={(e)=>setCardCode(e.target.value)}/>
      </label>
    </form>
    <StyledButton case="primary" style={{marginTop:'49px'}} onClick={()=>submit()}>Crear cuenta</StyledButton>
